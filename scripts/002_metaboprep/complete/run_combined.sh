...
#SBATCH --job-name=metaboprep-complete-combined
#SBATCH --nodes=1
...
# complete
cd ${DATA}/complete
Rscript ${SCRIPT} ${DATA}complete/parameter_file_combined.txt
mv ${IN}complete/metaboprep* ${OUT}complete/combined/
mv ${DATA}complete/figure/ ${OUT}complete/combined/
mv ${DATA}complete/metaboprep* ${OUT}complete/combined/
